...
public struct Token: Equatable, Sendable {
    ...
    static func word(_ t: String) -> Token {
        Token(text: t, kind: .word, intValue: nil, padWidth: nil, date: nil, dateFormat: nil)
    }
    static func number(_ t: String) -> Token {
        Token(text: t, kind: .number, intValue: Int(t), padWidth: t.count, date: nil, dateFormat: nil)
    }
    static func separator(_ t: String) -> Token {
        Token(text: t, kind: .separator, intValue: nil, padWidth: nil, date: nil, dateFormat: nil)
    }
    static func dateToken(_ raw: String, _ d: SimpleDate, _ f: DateFormatSig) -> Token {
        Token(text: raw, kind: .date, intValue: nil, padWidth: nil, date: d, dateFormat: f)
    }
}
    ...
    public static func tokenize(_ stem: String) -> [Token] {
        ...
        for dm in dates {
            if cursor < dm.range.lowerBound {
                tokens.append(contentsOf: classRuns(String(stem[cursor..<dm.range.lowerBound])))
            }
            tokens.append(.dateToken(dm.raw, dm.date, dm.format))
            cursor = dm.range.upperBound
        }
        if cursor < stem.endIndex {
            tokens.append(contentsOf: classRuns(String(stem[cursor..<stem.endIndex])))
        }
        return tokens
    }
